...
public class TileFetcher : MonoBehaviour
{
    ...
    void Awake()
    {
        // Force the grid extent to match the TerrainGenerator if it exists
        var tg = FindFirstObjectByType<TerrainGenerator>();
        if (tg != null)
        {
            elevationGridExtent = tg.terrainExtent * 2f;
            elevationGridSize = tg.gridSize;
        }
    }
    ...
    private IEnumerator FetchElevation(double lat, double lng)
    {
        ...
        var gps = FindObjectOfType<GPSTracker>();
        ...
        double lat, double lng, int heading, Dictionary<int, Texture2D> results)
    {
        ...
        // detect that by checking if it's nearly uniform grey
